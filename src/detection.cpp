		...
		while (camera.isOpened())
		{
			...
			if (!shapes.empty()) {
				...
				try {
					...
					cv::cvtColor(eyesRaw, eyesRaw, cv::COLOR_BGR2GRAY);// 彩色图转换成灰度图
					equalizeHist(eyesRaw, eyesRaw);//直方图均值化
				...
				//	原文链接：https ://blog.csdn.net/computerme/article/details/38142125
